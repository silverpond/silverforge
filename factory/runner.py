...
def _run_crucible(client: SSHClient, working_dir: str, base_branch: str, config: "CrucibleConfig") -> tuple:
    ...
    # Check diff size first — large diffs cause crucible to time out
    diff_size = client.run(
        f"git -C {working_dir} diff {base_branch} | wc -l",
        timeout=30,
    )
    try:
        diff_line_count = int(diff_size.stdout.strip())
    except (ValueError, AttributeError):
        diff_line_count = 0

    if diff_line_count >= _CRUCIBLE_DIFF_LIMIT:
        debug_output = _truncate_large_diff(client, working_dir, base_branch, diff_line_count)
        return "", "", debug_output
